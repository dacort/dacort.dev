...
Ah the dreaded `Context left until auto-compact` counter. I've had good luck here with telling Claude to just keep a `PLAN.md` file up-to-date, except in the case of larger projects. I _didn't_ do that for this project, as I saw Claude created its own plan file, and regretted it. Once the auto-compaction happened, Claude forgot about the plan but luckily I still had a copy of it in `~/.claude/plans` I could reference. I probably should have added a nice `CLAUDE.md` or direct it to write out the plan file locally, but alas. 
